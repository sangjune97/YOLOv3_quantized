...
import utils.utils
from quantize import fuse_model
backend = "fbgemm"  # running on a x86 CPU. Use "qnnpack" if running on ARM.

parser = argparse.ArgumentParser()
parser.add_argument("--image_folder", type=str, default="./test_images", help="path to image folder")
...
parser.add_argument("--quantized", type=bool, default=False, help="is model quantized")
...
if args.quantized:
        ...
        #fuse model
    model = model.yolov3_quant.YOLOv3(args.image_size, num_classes).to(device)
    model = fuse_model(model)

        #insert obsever
    for n, m0 in model.named_modules():
        if "yolo_layer" in n:
            pass
        else:
            m0.qconfig = torch.ao.quantization.get_default_qconfig(backend)
            torch.ao.quantization.prepare(m0, inplace = True)
    model=model.to('cpu')
    model.eval()
