...
class YOLOv3(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        with torch.no_grad():
            for key, module in self.darknet53.items():
                ...
                if module_type == 'conv':
                    ...
                    out = module(x)
                    x= self.skip_add.add(x,out)
                    
                    #x += out
                    if key == 'residual_3_8' or key == 'residual_4_8' or key == 'residual_5_4':
                        residual_output[key] = self.dequant(x)
            
        # Yolov3 layer forward
        conv_block3 = self.conv_block3(residual_output['residual_5_4'])
        scale3 = self.conv_final3(self.quant1(conv_block3))
        yolo_output3, layer_loss = self.yolo_layer3(self.dequant(scale3), targets)
        loss += layer_loss
        ...
        scale2 = self.conv_final2(self.quant2(conv_block2))
        yolo_output2, layer_loss = self.yolo_layer2(self.dequant(scale2), targets)
        loss += layer_loss
        ...
        scale1 = self.conv_final1(self.quant3(conv_block1))
        yolo_output1, layer_loss = self.yolo_layer1(self.dequant(scale1), targets)
        loss += layer_loss
