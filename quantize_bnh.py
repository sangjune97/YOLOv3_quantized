...
def quant_model(model, path, image_size, batch_size, num_workers, device):
    ...
    #torch.ao.quantization.convert(model.quant, inplace =True)

    #torch.ao.quantization.convert(model.darknet53, inplace =True)
    #torch.ao.quantization.convert(model.skip_add, inplace = True)
    return model
